...
const { fetchNaverAddressDetailed } = require('./naver');
...
async function reverseGeocode(lat, lon, config, options = {}) {
  ...
  const vworldResult = await fetchVworldAddressDetailed(latitude, longitude, config);
  const vworldAddress = vworldResult.address;
  let address = vworldAddress;
  let naverResult = { address: null, attempted: false, provider: 'naver', reason: 'not-requested' };
  let naverAddress = null;

  if (!address || (preferBuildingName && !address?.poiName)) {
    naverResult = await fetchNaverAddressDetailed(latitude, longitude, config);
    naverAddress = naverResult.address;
  }
  ...
  if (!address) {
    return {
      ...
      diagnostics: {
        vworld: {
          attempted: vworldResult.attempted,
          reason: vworldResult.reason,
          statusCode: vworldResult.statusCode || 0,
        },
        naver: {
          attempted: naverResult.attempted,
          reason: naverResult.reason,
          statusCode: naverResult.statusCode || 0,
        },
      },
    };
  }
...
module.exports = {
  reverseGeocode,
  fetchVworldAddressDetailed,
  fetchNaverAddressDetailed,
  translateLocation,
};
