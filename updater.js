...
async function getClusterAddress(client, cluster) {
    ...
    if (!address || config.appendBuildingName) {
        naverAddress = await fetchNaverAddress(cluster.centroidLat, cluster.centroidLon);
    }
    ...
    if (config.appendBuildingName && naverAddress?.poiName && !address.city.includes(naverAddress.poiName)) {
